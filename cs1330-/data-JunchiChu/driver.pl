...
open(INFILE, "./btest-zapped.out") 
    or die "$0: ERROR: could not open input file ./btest-zapped.out: $!\n";
...
while ($line = <INFILE>) {
    ...
    # This hack is to scale the assignment to 100 points
	$puzzle_p_points{$name} = $p_points * 2;
	$total_p_points += $p_points * 2;
...
#
open(INFILE, "./dpc-opcount.out") 
    or die "$0: ERROR: could not open input file ./dpc-opcount.out\n";
...
$autoresult = "VALID|$tpoints|$total_c_points|$total_p_points|$tops";
foreach $name (sort {$puzzle_number{$a} <=> $puzzle_number{$b}} 
        keys %puzzle_number) {

...
process Then we need to run execv to process our command!
                    ...
                    int res;
                    while ((res = waitpid(-1, &status, WNOHANG | WUNTRACED)) >
                           0) {
                         if (WIFEXITED(status)) {
                            printf(
                                "[%d] (%d) terminated with exit status \
"
                                "%d\n",
                                get_job_jid(myjoblist, res), res, status);
                            //remove_job_pid(myjoblist, res);
                        }
                    }
                    ...
                    /*
                       if (fg_bg == 1) {
                            fprintf(stderr, "[%d] (%d)\n", job, res);
                            add_job(myjoblist, job, res, RUNNING, input);
                            job++;
                        }
                    */
...
and the next token at.
                        ...
                        if (fg_bg == 0) {
                            int sta_w = 0;
                            sta_w = waitpid(-1, &status, WUNTRACED);
                            if (sta_w < 0) {
                        ...
                        if (WIFSIGNALED(status)) {
                            ...
                            printf("[%d] (%d) suspended by signal 20\n",
                                   job, child);
                        ...
                        /*
                            if (WIFSIGNALED(status)) {
                              printf("[1] (%d) terminated by signal 2\n",child);
                            } else if (WIFSTOPPED(status)) {
                              printf("[%d] (%d) stopped by signal
                           20\n",get_job_jid(myjoblist,child),child);
                              add_job(myjoblist, job, child, RUNNING, input);
                                  job++;

                            }else if(WIFEXITED(status)){
                              printf("exit normallysdasda\n");
                            }
                        */
                        pid_t shellid;
                        shellid = getpid();

...
int end_of_file_check(ssize_t m);
...
int main() {
	...
	if(checkread <0){
        ...
        if (end_of_file_check(checkread) == 1) {
            exit(0);
        }
...
process Then we need to run execv to process our command!
                    ...
                    if (bg_com == 1) {
                        ...
                        if (tcsetpgrp(STDIN_FILENO, getpgid(bg_pid)) < 0) {
                            perror("tcset fail");
                        }
                    ...
                    if (fg_com == 1) {
                        ...
                        if (job_id > 0) {
                            ...
                            if (tcsetpgrp(STDIN_FILENO, getpgid(fg_pid)) < 0) {
                                perror("tcset error");
                            }
                            int s;
...
void check_zombie(job_list_t *myjoblist) {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0) {
        if (WIFEXITED(status)) {
            fprintf(stderr, "[%d] (%d) dterminated with exit status %d\n",
...
int end_of_file_check(ssize_t checkread) {
    if (checkread == 0 || checkread == EOF) {
        cleanup_job_list(myjoblist);
